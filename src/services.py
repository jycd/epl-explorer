...
def get_team_matches_by_month_service(team_name, month, season=None):
    """Get all matches for a team in a specific month and optionally season"""
    files = sorted(glob.glob(os.path.join(DATA_DIR, "*_E0.csv")), reverse=True)
    
    # Filter by season if provided
    if season:
        files = [f for f in files if os.path.basename(f).startswith(season)]
    else:
        files = files[:10]  # Last 10 seasons if no season specified
    ...
    for f in files:
        ...
        # Filter by month (format: "January", "February", etc.)
        month_matches = team_matches[team_matches['Date'].dt.strftime('%B') == month]
        
        if month_matches.empty:
            continue
            
        for _, match in month_matches.iterrows():
            matches.append({
                "season": file_season,
                "date": match['Date'].strftime('%Y-%m-%d') if isinstance(match['Date'], pd.Timestamp) else match['Date'],
                "home_team": match['HomeTeam'],
